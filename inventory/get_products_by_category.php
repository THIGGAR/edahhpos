...
function sendError($message, $httpCode = 400, $details = null) {
    logError("Get Products by Category Error: $message", $details ? ['details' => $details] : []);
...
try {
    ...
    if (!empty($products)) {
        ...
        sendResponse(false, 'No products found in this category', []);
    }
} catch (Exception $e) {
    sendError('An error occurred while getting products: ' . $e->getMessage(), 500);
}
?>